...
class PandaChefEnv(object):
    def __init__(self, render=False, time_step = 1./150.,frame_skip=1):
        ...
        self.robot_id = bullet_client.loadURDF("./franka_panda_chef/panda_chef.urdf", np.array([0,0,0.5]), useFixedBase=True, flags=flags)
    ...
    def get_reward(self, ee_state, pizza_state, action):
        ...
        return catch_rew - np.linalg.norm(action)**2 + flip_rew + pan_reward
        ...
        for i in range(pandaNumDofs):
            if i in jnt_ctrl_idx:
                bullet_client.setJointMotorControl2(
                    self.robot_id, i, bullet_client.VELOCITY_CONTROL, targetVelocity=cmd[ctrl_idx], force=100)
                # bullet_client.setJointMotorControl2(
